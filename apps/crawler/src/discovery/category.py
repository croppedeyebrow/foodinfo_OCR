...
from ..url_parser import InvalidCategoryError, validate_category_inputs
from .base import (
    DiscoveryResult,
...
class CategoryProductDiscovery:
    ...
    def discover(
        ...
        *,
        ...
        try:
            code, absolute_url = validate_category_inputs(category_code, category_url)
        ...
        try:
            ...
            page.goto(absolute_url, wait_until="domcontentloaded", timeout=timeout_ms)
        ...
        if not products:
            failures.append(
                DiscoveryFailure(
                    ...
                    error_message="No product links found on category page",
                    failed_at=datetime.now(KST),
                )
            )
            status = "FAILED"
